...
import '../Map_Screen/driver_city_map.dart';
import '../profile.dart';

class DriverHomeScreen extends StatefulWidget {
  final String email;

  const DriverHomeScreen({Key? key, required this.email}) : super(key: key);
...
class _DriverHomeScreenState extends State<DriverHomeScreen> {
      ...
      if (snapshot.exists) {
        ...
        if (mounted) {
          setState(() {
            ...
            _fullName = user['fullName'] ?? 'Driver';
          });
        }
      }
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        children: [
          DriverCityMap(email: widget.email),
        ],
      ),
    );
  }
